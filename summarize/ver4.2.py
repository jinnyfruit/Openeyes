...
def measure_performance():
    text_lengths = [10, 50, 100, 200, 500]  # Input text lengths to experiment with
    model_names = ["sshleifer/distilbart-cnn-12-6", "lexrankr"]  # Models to compare

    summary_lengths = {model_name: [] for model_name in model_names}
    elapsed_times = {model_name: [] for model_name in model_names}

    for length in text_lengths:
        text = "A" * length  # Generate text of given length
        ...
        for model_name in model_names:
            ...
            # Store the summary length and elapsed time
            summary_lengths[model_name].append(len(summary.split()))
            elapsed_times[model_name].append(elapsed_time)

    return text_lengths, summary_lengths, elapsed_times

def plot_summary_performance(text_lengths, summary_lengths, elapsed_times):
    models = list(summary_lengths.keys())

    # Plot summary lengths
    plt.figure(figsize=(10, 5))
    for model in models:
        plt.plot(text_lengths, summary_lengths[model], label=model)
    ...
    plt.figure(figsize=(10, 5))
    for model in models:
        plt.plot(text_lengths, elapsed_times[model], label=model)
    plt.xlabel("Input Text Length")
...
# Measure the performance of the summarization models
text_lengths, summary_lengths, elapsed_times = measure_performance()

# Plot the summary performance
plot_summary_performance(text_lengths, summary_lengths, elapsed_times)
